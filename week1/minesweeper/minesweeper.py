...
class MinesweeperAI():
    ...
    def update_knowledge(self):
        ...
        for s1 in self.knowledge:
            if s1.cells == set():
                new_knowledge.remove(s1)
            for s2 in self.knowledge:
                if s2.cells == set():
                    new_knowledge.remove(s2)
                if s1 != s2:
                    if s2.cells.issubset(s1.cells):
                        new_cells = s1.cells.difference(s2.cells)
                        s_count = s1.count - s2.count
                        inference = Sentence(new_cells, s_count)
                        if inference not in new_knowledge:
